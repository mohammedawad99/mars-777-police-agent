...
| Sensitivity analysis | `PARTIAL` | grid size, quota and horizon are swept and reported per configuration family; a dedicated one-factor-at-a-time sensitivity table is not separately produced |
| Analysis notebook | `PENDING` | the guideline's *"or equivalent"* is met by tested analysis functions plus one documented regeneration command; a notebook surface is deferred |
